...
// matching-equity-table_client.js - WITH ALL-OR-NOTHING BULK CSV UPLOAD, AUDIT TRACKING, YELLOW HIGHLIGHTING
(() => {
  const API_BASE = "";
  ...
  function updateStats(entries) {
    ...
    const totalShares = entries.reduce((sum, e) => sum + (parseFloat(e.reported_shares) || 0), 0);

    document.getElementById('total-entries').textContent = totalEntries.toLocaleString();
    document.getElementById('total-investment-l').textContent = 
      'L ' + totalInvestmentL.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
    document.getElementById('total-investment-usd').textContent = 
    ...
    document.getElementById('total-shares').textContent = 
      totalShares.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
  }
  ...
  function renderTable(entries) {
    ...
    entries.forEach(entry => {
      ...
      row.innerHTML = `
        ...
        <td>${formatCurrency(entry.reported_shares)}</td>
        <td>${formatCurrency(entry.share_capital_multiplied)}</td>
        <td>${formatCurrency(entry.expected_profit_pct)}%</td>
        ...
        <td>
          <button class="action-btn edit-btn" onclick="window.editEntry(${entry.investment_id})">Edit</button>
          <button class="action-btn delete-btn" onclick="window.deleteEntry(${entry.investment_id})">Delete</button>
        </td>
      `;
  ...
  async function loadEntries(highlightId = null) {
    try {
      ...
      const resp = await fetch(`${API_BASE}/api/equity/matching/entries`, {
        credentials: 'include'
      });
  ...
  function downloadCSV() {
    const headers = [
      'Partner Name', 'Year', 'Reported Shares', 'Share Capital', 'Expected %',
      'Investment (L)', 'Investment ($)', 'Exchange Rate',
      'Proposal State', 'Transaction Type',
      'Jan (L)', 'Feb (L)', 'Mar (L)', 'Apr (L)', 'May (L)', 'Jun (L)',
      'Jul (L)', 'Aug (L)', 'Sep (L)', 'Oct (L)', 'Nov (L)', 'Dec (L)',
      'Technician', 'Business Category', 'Company Type',
      'Community', 'Municipality', 'State', 'Comments', 'File', 'Last Edited By', 'Last Edited'
    ];

    const rows = filteredEntries.map(entry => [
      entry.partner_name || '',
      entry.year || '',
      entry.reported_shares || '',
      entry.share_capital_multiplied || '',
      ...
      entry.state || '',
      entry.comments || '',
      extractFileFromNotes(entry.notes) || '',
    ...
    link.download = `matching_equity_entries_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  }
  ...
  // ============================================
  // BULK CSV UPLOAD - TEMPLATE DOWNLOAD (UPDATED)
  // ============================================
  function downloadTemplate() {
    ...
    const exampleRow = [
      ...
      '1000',                    // reported_shares (optional - positive number)
      '50000',                   // share_capital_multiplied (optional - positive number)
      ...
      'Approved',                // proposal_state (optional - e.g., Pending, Approved, Rejected)
      'New Investment',          // transaction_type (optional - e.g., New Investment, Follow-up, Exit)
    ...
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'matching_equity_template.csv';
    link.click();
  ...
  // ============================================
  // BULK CSV UPLOAD - SHOW RESULT (UPDATED)
  // ============================================
  ...
  async function uploadCSV(file) {
    ...
    try {
      ...
      const resp = await fetch(`${API_BASE}/api/equity/matching/bulk-upload`, {
        method: 'POST',
        body: formData,
        credentials: 'include'
      });
  ...
  if (uploadCsvLabel) {
    uploadCsvLabel.addEventListener('click', () => {
  ...
  function editEntry(id) {
    ...
    document.getElementById('edit-id').value = entry.investment_id;
    document.getElementById('edit-partner-name').value = entry.partner_name || '';
    document.getElementById('edit-year').value = entry.year || '';
    document.getElementById('edit-reported-shares').value = entry.reported_shares || '';
    document.getElementById('edit-share-capital').value = entry.share_capital_multiplied || '';
  ...
  if (editForm) {
    editForm.addEventListener('submit', async (e) => {
      ...
      const updatedData = {
        ...
        reported_shares: parseFloat(document.getElementById('edit-reported-shares').value) || null,
        share_capital_multiplied: parseFloat(document.getElementById('edit-share-capital').value) || null,
        ...
        proposal_state: document.getElementById('edit-proposal-state').value || null,
        transaction_type: document.getElementById('edit-transaction-type').value || null,
        // Monthly distributions
        ...
        technician: document.getElementById('edit-technician').value.trim() || null,
        business_category: document.getElementById('edit-business-category').value.trim() || null,
        company_type: document.getElementById('edit-company-type').value.trim() || null,
        community: document.getElementById('edit-community').value.trim() || null,
        municipality: document.getElementById('edit-municipality').value.trim() || null,
        state: document.getElementById('edit-state').value.trim() || null,
        comments: document.getElementById('edit-comments').value.trim() || null
      };

      try {
        const resp = await fetch(`${API_BASE}/api/equity/matching/entry/${id}`, {
          method: 'PUT',
          headers: {
